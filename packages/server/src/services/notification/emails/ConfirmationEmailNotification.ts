...
export default class ConfirmationEmailNotification implements IConfirmationEmailNotification {
    ...
    public async send(email: string, confirmationCode: number): Promise<void> {
        await this._notificationService.send({
            ...
            to: {
                email,
                name: Translations.text('HELLO_GUEST'),
            },
            subject: Translations.text('CONFIRM_MAIL_ADDRESS'),
            template: this._mailTemplateService.getConfirmMailTemplate(),
            text: Translations.text('CONFIRM_MAIL_TEXT'),
            tags: {
                code: confirmationCode,
                CONFIRM_MAIL_ADDRESS: Translations.text('CONFIRM_MAIL_ADDRESS'),
                HELLO_GUEST: Translations.text('HELLO_GUEST'),
                CONFIRM_MAIL_TEXT: Translations.text('CONFIRM_MAIL_TEXT'),
                CONFIRM_MAIL_TEXT2: Translations.text('CONFIRM_MAIL_TEXT2'),
                SINCERELY: Translations.text('SINCERELY'),
            },
        });
    }
}
